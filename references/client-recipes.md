...
Conventions:
- `BASE_URL` examples below use `https://share.combos.fun` (replace with real host).
...
```bash
curl -sS -X POST "https://share.combos.fun/api/upload" \
...
```powershell
curl.exe -sS -X POST "https://share.combos.fun/api/upload" `
...
```bash
curl -sS -X POST "https://share.combos.fun/api/publish" \
  -H "Authorization: Bearer $AUTH_TOKEN" \
...
```powershell
$baseUrl = "https://share.combos.fun"
$form = @{
  title = "My Game"
  description = "Generated by AI"
  platform = "desktop"
...
```bash
curl -sS "https://share.combos.fun/api/games/random?count=6&platform=desktop&contentType=game"
```

Fetch detail (includes `claimTokenState` when `onceToken` provided):

```bash
curl -sS "https://share.combos.fun/api/games/<id>?fingerprint=device-123&onceToken=<onceToken>"
```

Claim ownership:

```bash
curl -sS -X POST "https://share.combos.fun/api/games/<id>/claim" \
  -H "Authorization: Bearer $AUTH_TOKEN" \
  -H "Content-Type: application/json" \
